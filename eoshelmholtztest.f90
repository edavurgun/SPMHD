      ...
      real*8 temp,dens,xm,am,zm,abar,zbar
!      integer ionmax,jlo_eos,jhi_eos
      integer ionmax
      parameter(ionmax=3)
      dimension xm(ionmax),am(ionmax),zm(ionmax)
!      dimension temp_row(1),den_row(1),abar_row(1),zbar_row(1)
      xm(1)=0.75
      xm(2)=0.23
      xm(3)=0.02
      am(1)=1.0d0
...
!     average atomic weight and charge
      abar=1.0d0/sum(xm(1:ionmax)/am(1:ionmax))         
      zbar=abar*sum(xm(1:ionmax)*zm(1:ionmax)/am(1:ionmax))     
      temp_row(1)=1.d08
      den_row(1)=1.d06 
      abar_row(1)=abar
      zbar_row(1)=zbar
